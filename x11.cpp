                 ...
                 unsigned long *nr_items)
{
    ...
    int format, status;
    status = XGetWindowProperty(QX11Info::display(), window, atom, 0L, ~(0L), False, atom_type,
                                &type_ret, &format, nr_items, &bytes_after_return,
...
QVector<unsigned long> x11ClientList(void)
{
    ...
    initAtoms();
    xgwp(QX11Info::appRootWindow(), netClientList, XA_WINDOW, reinterpret_cast<char **>(&data),
         &nr_items);
...
bool x11IsHidden(unsigned long window)
{
    initAtoms();
    unsigned long *data = 0, nr_items;
    xgwp(window, netWmWindowType, XA_ATOM, reinterpret_cast<char **>(&data), &nr_items);
    if (!data)
        return false;
    for (unsigned int i = 0; i < nr_items; i++) {
        if (data[i] == netWmWindowTypeDock || data[i] == netWmWindowTypeDesktop
            || data[i] == netWmWindowTypeToolbar || data[i] == netWmWindowTypeMenu
